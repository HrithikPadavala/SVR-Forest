...
class tree(node):
    ...
    def fit_sv(self, X, Y, D,pen, feature_select=False, c0=1, weight=1, border=None, standardize=False, 
        ...
        sv_reg_min = self.sv_regular(surface, volume, d,D)
                ...
                for sa in range(len(node.Y)-1):    ## sa is short for sample                        
                    ...
                    if (dat[sa+1][0] != dat[sa][0]):
                        ...
                        if volume_new_lst[0] <= 0 or surface_new_lst[0] <= 0:
                            ...
                            svr = self.sv_regular(surface_new_lst[0], volume_new_lst[0], d,D) 
                        ...
                        if volume_new_lst[1] <= 0 or surface_new_lst[1] <= 0:
                            ...
                            svr = self.sv_regular(surface_new_lst[1], volume_new_lst[1], d,D) 
                        ...
                        if volume_new_lst[2] <= 0 or surface_new_lst[2] <= 0:
                            ...
                            svr = self.sv_regular(surface_new_lst[2], volume_new_lst[2], d,D)
                        ...
                        if volume_new_lst[3] <= 0 or surface_new_lst[3] <= 0:
                            ...
                            svr = self.sv_regular(surface_new_lst[3], volume_new_lst[3], d,D)
    ...
    def data_standardize(self, X):
        ...
        for j in range(d):
            ...
            if feat_max == feat_min:
                raise Exception('feature '+str(j)+' has only one value')
            border_dist = (int(int(feat_max))-int(int(feat_min)))/(n-1)
            border[j,:] = [feat_min-border_dist, feat_max+border_dist]    
        shifts = - border[:,0]
        multipliers = np.diag(1/(border[:,1]-border[:,0]))
    ...
    @staticmethod
    def sv_regular(surface, volume, d,D):
        ''' Compute surface-to-volume regularization. '''
        
        return (surface/(volume+D))
